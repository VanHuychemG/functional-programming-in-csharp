...
{
    /// <summary>
    /// Extensions for async operations where the task appears in the both operands
    /// </summary>
    public static class AsyncResultExtensionsBothOperands
    {
        ...
        public static async Task<Result<TK>> Map<T, TK>(this Task<Result<T>> resultTask, Func<T, Task<TK>> func)
        {
            var result = await resultTask.ConfigureAwait(false);
            if (result.IsFailure)
                return Result.Fail<TK>(result.Error);
        ...
        {
            var result = await resultTask.ConfigureAwait(false);return await func(result).ConfigureAwait(false);
        }
